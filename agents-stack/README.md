...
- `CRW_PATH`: checkout of the crw source repo (build-from-source pattern, same as `GPT_RESEARCHER_PATH`/`OPENCRAWL_PATH`/`CAMOFOX_BROWSER_PATH`); the compose `build.context` points here and builds with `CARGO_PKGS="-p crw-server --features camoufox"` (server only — adding `-p crw-mcp`/`-p crw-cli` re-enables `cdp` via cargo feature unification, since both pull `crw-server`/`crw-renderer` with `features = ["cdp"]`). If the build fails with "none of the selected packages contains this feature", use the qualified form `-p crw-server --features crw-server/camoufox`.
